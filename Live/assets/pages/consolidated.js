  ...
  body:function(s){
    ...
    h += '<div class="qp-row">' +
      area('development','Development','Business units · capital projects','var(--area-dev-bg)', lead('Work Performed', M(2080.0), 'M', null, 'Value of work completed on development projects in the selected period, year to date. Stated in '+f.SAR+' millions.'),
        QP.tile('Commitments · Total', M(cTot), '<span class="qp-comp" role="group" aria-label="Commitments by unit" style="flex:1 1 100%;display:flex">'+COMMIT.map(function(c){ var tip = c.n+': '+f.SAR+' '+f.n(c.v, 1)+' M · '+Math.round(c.v / cTot * 100)+'% of commitments'; return '<i style="flex:'+c.v+';background:'+c.c+'" tabindex="0" data-tip="'+QP.esc(tip)+'" aria-label="'+QP.esc(tip)+'"></i>'; }).join('')+'</span>', 'neu', 'wide') +
